...
BEGIN
  ...
  IF v_service_name <> 'CCGL_MIG' THEN
    raise_application_error(-20082, 'Wrong service: ' || v_service_name || ', expected CCGL_MIG');
  END IF;
END;
/

PROMPT === 1) 收口门禁：APP窗口READY + 最近24小时监控绿态 ===
DECLARE
  v_app_window_ready NUMBER := 0;
  v_total_cnt        NUMBER := 0;
  v_bad_cnt          NUMBER := 0;
BEGIN
  SELECT COUNT(*)
    INTO v_app_window_ready
    FROM MIG_P1_BATCH_LOG
   WHERE batch_no = 'P1_BATCH01'
     AND object_name = 'APP_GRAY_WINDOW'
     AND action_type = 'STEP4_APP_WINDOW_READY'
  ...
  IF v_app_window_ready = 0 OR v_total_cnt = 0 OR v_bad_cnt > 0 THEN
    raise_application_error(
      -20083,
      'app route switch blocked: window_ready=' || v_app_window_ready
      || ', total=' || v_total_cnt || ', bad=' || v_bad_cnt
    );
  END IF;
END;
/

PROMPT === 2) 写入批次日志（应用路由切换完成） ===
MERGE INTO MIG_P1_BATCH_LOG t
USING (
  SELECT 'P1_BATCH01' AS batch_no,
         'CONTROL' AS object_type,
         'APP_ROUTE_SWITCH' AS object_name,
         'APP_ROUTE_SWITCH_DONE' AS action_type,
         'DONE' AS execute_status,
         'app route switched to new write path for PLAN_CUST and TIT02' AS note
  FROM dual
) s
ON (
...
FROM MIG_P1_BATCH_LOG
WHERE batch_no = 'P1_BATCH01'
  AND object_name = 'APP_ROUTE_SWITCH'
