...
DECLARE
  v_has_customers   NUMBER := 0;
  v_has_area_table  NUMBER := 0;
  ...
  v_sql            CLOB;
BEGIN
  SELECT COUNT(*) INTO v_has_customers
    FROM user_tables
   WHERE table_name = 'TMM22_CUSTOMERS';

  SELECT COUNT(*) INTO v_has_area_table
    FROM user_tables
   WHERE table_name = 'TIT01_TIMEPOINT_AREA';

  IF v_has_customers = 0 OR v_has_area_table = 0 THEN
    raise_application_error(
      -20003,
      'Missing source tables in current schema. Required: TMM22_CUSTOMERS and TIT01_TIMEPOINT_AREA'
    );
  END IF;
...
PROMPT === 完成：请继续执行 DBA_VERIFY_TIT01_TIMEPOINT_CUST_MIG.sql ===
