...
BEGIN
  ...
  IF v_service_name <> 'CCGL_MIG' THEN
    raise_application_error(-20073, 'Wrong service: ' || v_service_name || ', expected CCGL_MIG');
  END IF;
END;
/

PROMPT === 1) 写切换门禁（B10成功 + 最近24小时无异常） ===
DECLARE
...
BEGIN
  SELECT COUNT(*),
         SUM(CASE WHEN compare_result = 'MATCH' THEN 1 ELSE 0 END),
         SUM(CASE WHEN compare_result IN ('DIFF', 'ERROR') THEN 1 ELSE 0 END)
    INTO v_total_cnt, v_match_cnt, v_bad_cnt
  ...
  SELECT COUNT(*)
    ...
    FROM MIG_P1_SHADOW_WRITE_LOG
   WHERE batch_no = 'P1_BATCH01'
     AND source_object = 'PLAN_CUST'
     AND action_type = 'GRAY_B10_SUMMARY'
     AND compare_result = 'MATCH';

  IF v_total_cnt < 10 OR v_bad_cnt > 0 OR v_b10_ok = 0 THEN
    raise_application_error(
      -20074,
      'write cutover blocked: total=' || v_total_cnt || ', bad=' || v_bad_cnt || ', b10_ok=' || v_b10_ok
    );
  END IF;
END;
/
...
USING (
  SELECT 'P1_BATCH01' AS batch_no,
         ...
         'WRITE_CUTOVER_PLAN_CUST' AS object_name,
         'STEP3_3_WRITE_CUTOVER' AS action_type,
         'DONE' AS execute_status,
         'plan_cust write cutover approved; app route switch to new path' AS note
  FROM dual
) s
ON (
...
WHERE batch_no = 'P1_BATCH01'
  AND object_name = 'WRITE_CUTOVER_PLAN_CUST'
