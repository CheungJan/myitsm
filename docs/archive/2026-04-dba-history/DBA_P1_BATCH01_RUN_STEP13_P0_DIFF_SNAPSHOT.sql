...
BEGIN
  ...
  IF v_service_name <> 'CCGL_MIG' THEN
    raise_application_error(-20080, 'Wrong service: ' || v_service_name || ', expected CCGL_MIG');
  END IF;
END;
/

PROMPT === 1) 创建差异快照表（不存在则创建） ===
BEGIN
  EXECUTE IMMEDIATE '
...
PROMPT === 4) 写入批次日志（P0 差异快照落表） ===
MERGE INTO MIG_P1_BATCH_LOG t
USING (
  SELECT 'P1_BATCH01' AS batch_no,
         'CONTROL' AS object_type,
         'P0_DIFF_SNAPSHOT' AS object_name,
         'STEP0_DIFF_SNAPSHOT' AS action_type,
         'DONE' AS execute_status,
         'baseline vs dict diff snapshot persisted' AS note
  FROM dual
) s
ON (
