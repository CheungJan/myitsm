...
BEGIN
  ...
  IF v_service_name <> 'CCGL_MIG' THEN
    raise_application_error(-20181, 'Wrong service: ' || v_service_name || ', expected CCGL_MIG');
  END IF;
END;
/

PROMPT === 1) 前置门禁：BATCH04 必须存在 PLANNED 对象 ===
DECLARE
...
BEGIN
  SELECT COUNT(*)
   ...
   WHERE batch_no = 'P1_BATCH04'
     AND wave_no = 'WAVE_01'
     AND status = 'PLANNED';

  IF v_planned_cnt = 0 THEN
    raise_application_error(-20182, 'BATCH04 planned objects not found, cnt=0');
  END IF;
END;
/
