...
BEGIN
  ...
  IF v_service_name <> 'CCGL_MIG' THEN
    raise_application_error(-20171, 'Wrong service: ' || v_service_name || ', expected CCGL_MIG');
  END IF;
END;
/
...
USING (
  ...
  prior_used AS (
    ...
    WHERE batch_no IN ('P1_BATCH01', 'P1_BATCH02', 'P1_BATCH03')
  ),
  candidates AS (
