...
| code_typ | 用途 | 码值 |
|----------|------|------|
| OW | 资产所属 | 01商用电子/02通方信息/03门店资产/04海晟 |
| PF | 设备来源 | 00商用仓库/01门店移机/02烟草直调/03IT公司/04海晟公司 |
| BM_S | 业务模式 | 01销售/02租赁/03借用/04代维/05寄售/06试用/07免费投放/08合作运营 |
| SR | 服务责任方 | 01内部/02厂商/03代维 |
| ES | EID设备状态 | 0新品已检/1已使用/2已报废/3待检/4已退回/5返修中/6检验中/7生产中/8在库/9出库/S销售出库 |
| ET | 设备类型 | 0配件/1整机 |
| ETK | EID变更类型 | A属性变更/C客户分配/R回收/T客户转移 |
| AT | 资产类型 | 01新机/02旧机/03翻新机/04报废 |
| AS | 资产状态 | ACTIVE使用中/RETURNED已退回/SCRAPPED已报废 |
| C_TYPE | 物料收费类型 | 1配件更换/2购买/3纯服务费/4整机更换/5耗材线材 |
| IV | 入库类型 | 1采购入库/11质检入库/12退料入库/2销售退货入库/3服务返还入库/4调拨入库/5借出归还入库/6翻新入库/7回收入库/8生产入库/9返修入库 |
| OV | 出库类型 | 1销售出库/10翻新出库/11定额领料/12不良补料/2服务领用出库/3调拨出库/4借出出库/5质检出库/6退货出库/7报废出库/8生产出库/9返修出库 |
| ZT | 工单状态 | 1新建/2已派工/3关单(历史)/4未解决/5已解决/关单/6转修/7待配件/9作废 |
| CLO_REASON | 关闭原因 | 1正常解决/2客户拒修/3非设备问题/4重复报修/5转其他团队 |
| PAY_SVC | 服务费类型 | 01上门费/02检测费/03软件服务费/04其他 |
| PAY_CONS | 耗材类型 | 01线材/02枪线/03扫描枪线/04耗材/05 3G电源/06打印机电源/07其他 |
| OD | 质保范围 | 12新品质保/3旧品质保 |
| FL | 工单类型 | 1硬件故障/2软件故障/3其他/4网络故障/9巡检/BG设备变更/BY日常保养/GB门店关闭/GH免费更换/MD日常维护/MO新机开通/MR旧机翻新 |

> **ES 字典 '1' 双义说明**：`sflg='1'`（已使用）按 `whcd` 区分两种状态：
> - `whcd` = 工程师仓 → 持有中（服务领用出库后，工程师持有未安装）
> - `whcd` = NULL → 已安装（关单 L4 后，配件装到设备上）
>
> 该双义为重构系统新增（PB 原义 '1'=已使用/已安装），不新增码值，靠 whcd 物理位置区分。
