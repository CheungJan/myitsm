<template>
  <div class="page">
    <ItsmDetailLayout>
      <template #search>
        <div class="page-header"><h2>门店关闭</h2></div>
      </template>
      <template #list>
        <el-table :data="items" v-loading="loading" stripe size="small" highlight-current-row @row-click="open" row-key="store_close_id">
          <el-table-column prop="store_close_id" label="关店单号" width="120"/>
          <el-table-column label="关闭类型" width="90"><template #default="{row}">{{ clLabel(row.close_type as string) }}</template></el-table-column>
      ...
      <template #summary>
        <el-descriptions v-if="detail" :column="2" border size="small">
          <el-descriptions-item label="单号">{{ detail.store_close_id }}</el-descriptions-item>
          <el-descriptions-item label="类型">{{ clLabel(detail.close_type as string) }}</el-descriptions-item>
          ...
          <el-descriptions-item label="关闭类型">{{ clLabel(detail.close_type as string) }}</el-descriptions-item>
          <el-descriptions-item label="临时关闭开始">{{ detail.temp_close_date_begin || '-' }}</el-descriptions-item>
          <el-descriptions-item label="临时关闭结束">{{ detail.temp_close_date_end || '-' }}</el-descriptions-item>
      ...
      <template #tabs>
        <el-tabs v-if="detail" v-model="activeTab" type="border-card" size="small">
          ...
          <ItsmSubTablePanes :maintenance-id="(detail.store_close_id as string) || ''" :area-cd="((detail as any).area_cd as string) || ''" :current-status="((detail as any).current_status as string) || ''" :main-record="detail" />
    ...
    <el-dialog title="编辑门店关闭" v-model="showEdit" width="450px" :close-on-click-modal="false">
...
import {fetchStoreClose, transitionStoreClose, updateStoreClose} from '@/api/itsm'
...
const {dictLabel:clLabel} = useDict('CL')
...
async function doEdit() {
  try {
    await updateStoreClose(detail.value?.store_close_id as string, editForm.value)
...
async function doTransition(row:MntRecord,toStatus:string) {
  try {
    await transitionStoreClose(row.store_close_id as string, {to_status: toStatus})
