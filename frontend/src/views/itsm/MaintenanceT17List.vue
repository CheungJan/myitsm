    ...
    <ItsmDetailLayout>
      ...
      <template #summary>
        <el-descriptions v-if="detail" :column="3" border size="small">
          ...
          <el-descriptions-item label="工程师">{{ userName(detail.firstor as string) }}</el-descriptions-item>
      ...
      <template #tabs>
        <el-tabs v-if="detail" v-model="activeTab" type="border-card" size="small">
          ...
          <ItsmSubTablePanes :maintenance-id="(detail.daily_maintenance_id as string) || ''" :area-cd="((detail as any).area_cd as string) || ''" :current-status="((detail as any).current_status as string) || ''" :main-record="detail" />
...
<script setup lang="ts">import {ref} from 'vue';import {ElMessage} from 'element-plus';import AppPagination from '@/components/common/AppPagination.vue';import {useListPage} from '@/composables/useListPage';import {useDetailDrawer} from '@/composables/useDetailDrawer';import {useUserNames} from '@/composables/useUserNames';import {useCustomerCards} from '@/composables/useCustomerCards';import {fetchMaintenanceT17, transitionMaintenanceT17} from '@/api/itsm';import type {MntRecord} from '@/api/itsm';import ItsmDetailLayout from './ItsmDetailLayout.vue';import CustomerInfoTab from './CustomerInfoTab.vue';import AssetTab from './AssetTab.vue';import ItsmSubTablePanes from './ItsmSubTablePanes.vue'
...
function doSearch(){const p:Record<string,string>={};if(searchStatus.value)p.status=searchStatus.value;onSearch(p)}
