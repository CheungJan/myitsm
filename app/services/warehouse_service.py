...
def _enrich_warehouse_names(rows: list[dict[str, Any]]) -> None:
    ...
    if not whcds:
        return
    wh_map = dict(
        db.session.query(Warehouse.whcd, Warehouse.whnm).filter(Warehouse.whcd.in_(whcds)).all()
    )
    for r in rows:
        cd = r.get("whcd")
...
class StockInService:
    ...
    @staticmethod
    def skip_service_return(
        maintenance_id: str, reason: str, eids: list[str] | None = None
    ) -> dict[str, object]:
        """按EID标记ITSM配件变更为不入库。eids为空则标记该工单全部。"""
        from app.models.itsm import AccessoriesUpdate

        query = db.session.query(AccessoriesUpdate).filter(
    ...
    def confirm_service_return(maintenance_ids: list[str], operator: str) -> dict[str, object]:
        ...
        for mid in maintenance_ids:
            ...
            if details:
                ...
                eid_info = {
                    r[0]: r[1]
                    for r in db.session.query(EidModel.eid, EidModel.itemcd)
    ...
    def _validate_details(details: list[dict[str, Any]], invtyp: str = "") -> str | None:
        ...
        from app.models.master import Eid, Item, SysCode

        valid_qc = {
            c.code_cd
            for c in db.session.query(SysCode.code_cd).filter(SysCode.code_typ == "QC").all()
        }
        ...
        for i, d in enumerate(details, 1):
            ...
            if eid:
                eid_rec = (
                    db.session.query(Eid.itemcd, Eid.eid, Eid.whcd).filter(Eid.eid == eid).first()
                )
                if eid_rec is None:
                ...
                if check_store_off:
                    from app.models.master import Customer, CustPosRl
                    ...
                    if active_pos:
                        ...
                        if active_pos.cust_cd:
                            cust = (
                                db.session.query(Customer.cust_nm)
                                .filter(
                            ...
                            if cust:
                                location = (
                                    f"{cust}({active_pos.cust_cd})" if location else f"客户 {cust}"
                                )
                        if not location:
                            location = "客户现场"
    ...
    def create(
        ...
        ref_rgstbillid = (
            data.get("refbillid") if data.get("invtyp") in ("1", "4", "5", "8", "9") else None
        )
    ...
    def audit(
        ...
        for detail in record.details:  # type: ignore[attr-defined]
            ...
            StockDetailRepository.add_movement(
                ...
                billid=record.inbillid,
                invtyp=record.invtyp or "",
                iotyp="1",
                operator=auditor,
                itemtyp=getattr(detail, "itemtyp", None),
                prddate=getattr(detail, "prddate", None),
        ...
        if record.invtyp == "11" and record.refbillid and record.refbillid.startswith("QC"):
            try:
                ...
                for dt in record.details:
                    ...
                    d: dict[str, Any] = {
                        "itemcd": dt.itemcd,
                        "outqty": dt.inqty or 1,
                        "itemtyp": it,
                    }
                    if getattr(dt, "prddate", None):
                        d["prddate"] = (
                            dt.prddate.isoformat()
                            if hasattr(dt.prddate, "isoformat")
                            else str(dt.prddate)
                        )
        ...
        if record.invtyp == "9" and record.refbillid:
            ...
            for eid_val, itemcd_val in in_eids:
                db.session.query(EidModel).filter(
                    EidModel.itemcd == itemcd_val,
                    EidModel.eid == eid_val,
                ).update(
                    {
                        "whcd": record.whcd,
                        "sflg": "3",  # 待检
        ...
        if record.invtyp == "1" and record.refbillid:
            ...
            order_lines = (
                db.session.query(PurchaseRegisterDt.lineno, PurchaseRegisterDt.rgsqty)
                .filter(PurchaseRegisterDt.rgstbillid == record.refbillid)
                .all()
            )
            inqty_map = {d.reflineno: d.inqty or 0 for d in record.details}  # type: ignore[attr-defined]
            all_full = all(inqty_map.get(ol.lineno, 0) >= (ol.rgsqty or 0) for ol in order_lines)
    ...
    def update(
        ...
        if details is not None:
            ...
            ref_rgstbillid = (
                record.refbillid if record.invtyp in ("1", "4", "5", "8", "9") else None
            )
            for idx, detail_data in enumerate(details, start=1):
                if ref_rgstbillid:
                    detail_data["ref_rgstbillid"] = ref_rgstbillid
...
class StockOutService:
            ...
            for d in detail_list:  # type: ignore[var-annotated]
                ...
                try:
                    ...
                    if wo:
                        ...
                        bom_row = (
                            db.session.query(BomDt)
                            .filter(
                                BomDt.bomcd == wo.item_cd,
                                BomDt.itemcd == item_cd,
                            )
                            .first()
                        )
                        if bom_row:
    ...
    @staticmethod
    def audit(
        outbillid: str, auditor: str, auditflg: str = "2", checkmemo: str = ""
    ) -> dict[str, object]:
        record = StockOutRepository.get_by_id(outbillid)
        if record is None:
        ...
        if auditflg == "8":
            ...
            if record.invtyp == "8":
                ...
                db.session.query(QcResultDt).filter(
                    ...
                    {"replenish_status": "", "replenish_ov_billid": ""}, synchronize_session=False
                )
        ...
        for detail in record.details_eid:  # type: ignore[attr-defined]
            if detail.eid:
                ...
                eid_wh = db.session.query(Eid.whcd).filter(Eid.eid == detail.eid).scalar()
        ...
        if record.invtyp == "3" and getattr(record, "targetwhcd", None):
            from app.models.warehouse import StockOutDetailEid as OutEid
            from app.models.warehouse import StockOutDetailPrd as OutPrd
            ...
            for d in record.details_prd:
                ...
                in_details.append(
                    {
                        "itemcd": dd.get("itemcd"),
                        "inqty": dd.get("outqty", 0),
        ...
        if record.invtyp == "1":
            # 方案 A：出库审核通过后释放 EID 预占（reserve_planno 清空）
            from app.models.master import Eid as _EidReserve

            for d in record.details_eid:  # type: ignore[attr-defined]
                if d.eid:
                    db.session.query(_EidReserve).filter(
        ...
        for detail in record.details_eid:
            ...
            if detail.eid:
                ...
                db.session.query(Eid).filter(
                    ...
                    {"whcd": record.whcd, "sflg": "1", "qcflg": None}, synchronize_session=False
                )
        # 回退库存 prd
    ...
    def void(outbillid: str, operator: str) -> dict[str, object]:
        ...
        if linked_audited:
            return {
                "success": False,
                "error": f"关联入库单 {linked_audited.inbillid} 已审核且库存已入，不可作废",
            }
        record.auditflg = "V"
        record.opercd = operator
    ...
    def close_lines(
        ...
        if closed_count > 0:
            ...
            closed_linenos = {
                line.get("lineno") for line in lines if line.get("lineno") is not None
            }
            # 查找关联的 IV=9 未审核草稿
            drafts = (
    ...
    def update(
        ...
        if outdate:
            record.outdate = (
                dt_parse.fromisoformat(outdate) if isinstance(outdate, str) else outdate
            )
        if details_eid is not None or details_prd is not None:
            ...
            if details_eid:
                for idx, d in enumerate(details_eid, start=1):
                    StockOutRepository.add_detail_eid(
                        outbillid=record.outbillid, whcd=record.whcd, lineno=idx, data=d
                    )
            if details_prd:
...
class StockBalanceService:
    ...
    def list_movements(
        ...
        total = query.count()
        items = (
            query.order_by(StockDetailDt.gendate.desc())
    ...
    def inventory_summary(
        whcd: str | None = None, period: str = "", page: int = 1, per_page: int = 20
    ) -> dict[str, Any]:
        ...
        month_q = (
            db.session.query(
                ...
                func.sum(case((StockDetailDt.iotyp == "1", StockDetailDt.itemqty), else_=0)).label(
                    "in_qty"
                ),
                func.sum(
                    case((StockDetailDt.iotyp == "0", func.abs(StockDetailDt.itemqty)), else_=0)
                ).label("out_qty"),
            )
            .filter(
            ...
            .group_by(StockDetailDt.whcd, StockDetailDt.itemcd)
            .subquery()
        )
        ...
        q = db.session.query(
            ...
            month_q.c.in_qty,
            month_q.c.out_qty,
            (func.coalesce(begin_q.c.begin_qty, 0) + month_q.c.in_qty - month_q.c.out_qty).label(
                "end_qty"
            ),
        ).outerjoin(
            begin_q,
        ...
        total = q.count()
        rows = (
            q.order_by(month_q.c.whcd, month_q.c.itemcd)
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
        )
        ...
        return {
            ...
            "page": page,
            "per_page": per_page,
            "period": period or pd.strftime("%Y-%m"),
        }

    @staticmethod
    def daily_snapshot(
        whcd: str | None = None, date_str: str = "", page: int = 1, per_page: int = 20
    ) -> dict[str, Any]:
        ...
        total = q.count()
        rows = (
            q.order_by(StockDetailDt.whcd, StockDetailDt.itemcd)
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
        )
        ...
        snap_map = {}
        if item_keys:
            for sd_row in db.session.query(SD).filter(SD.whcd.in_([k[0] for k in item_keys])).all():
    ...
    @staticmethod
    def inventory_aging(
        whcd: str | None = None, page: int = 1, per_page: int = 20
    ) -> dict[str, Any]:
        ...
        first_in = (
            ...
            .filter(StockDetailDt.useflg == "1", StockDetailDt.iotyp == "1")
            .group_by(StockDetailDt.whcd, StockDetailDt.itemcd)
            .subquery()
        )
        ...
        total = q.count()
        rows = (
            q.order_by(StockDetail.whcd, StockDetail.itemcd)
            .offset((page - 1) * per_page)
            .limit(per_page)
            .all()
        )
...
class AssetCheckService:
    ...
    def get(opbillid: str) -> dict[str, Any] | None:
        ...
        if record is None:
            return None
        result = record.to_dict()
        result["details"] = [d.to_dict() for d in record.details.filter_by(useflg="1").all()]
        return result

    @staticmethod
    def list_records(page: int = 1, per_page: int = 20, useflg: str | None = "1") -> dict[str, Any]:
        items, total = AssetCheckRepository.list_all(page=page, per_page=per_page, useflg=useflg)
        return {
...
class PosChangeService:
    ...
    @staticmethod
    def list_records(page: int = 1, per_page: int = 20, useflg: str | None = "1") -> dict[str, Any]:
        items, total = PosChangeRepository.list_all(page=page, per_page=per_page, useflg=useflg)
        return {
            "items": [item.to_dict() for item in items],
            "total": total,
            "page": page,
...
class OverLostService:
    ...
    def create(
        ...
        for idx, detail_data in enumerate(details, start=1):
            OverLostRepository.add_detail(olbillid=record.olbillid, lineno=idx, data=detail_data)
        for idx, eid_data in enumerate(eid_details, start=1):
            OverLostRepository.add_eid_detail(olbillid=record.olbillid, lineno=idx, data=eid_data)
        db.session.commit()
        return record.to_dict()
