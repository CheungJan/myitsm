...
class PlanCust(BaseModel):
    ...
    pos_item = db.Column(db.String(6), comment="目标机型（新开店要安装的整机型号）")
    posid = db.Column(db.String(13), comment="目标设备EID")
    ...
    solve_type = db.Column(db.String(2), comment="旧机处理方式（翻新场景）")
    back_status = db.Column(db.String(2), comment="回退状态")
    cust_useflg = db.Column(db.String(2), comment="源门店无效化（移机/取机时勾选使源门店useflg=0）")
    ...
    is_outflag = db.Column(db.String(2), comment="出库标志（1=已生成OV=1出库单）")
