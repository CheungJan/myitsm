...
| 阶段 | 状态 | 内容 |
|------|------|------|
| 阶段1 | **已完成** | 基础架构（Flask工厂+SQLAlchemy模型+JWT认证+统一状态机） |
| 阶段2 | **已完成** | ITSM核心迁移（30+表+Repository/Service/API+P0优化1-4） |
| 阶段3 | **待开始** | 仓储统一模型(TWH*) + 采购(TPC*) + 销售(TSL*) + **[Tier-1] SLA服务级别管理** |
| 阶段4 | 待开始 | 辅助模块（考勤TKQ/库存预警TIV/价格TIP/调拨/考核）+ **[Tier-1] 合同管理(THT01) + 发票管理(TAC01) + 通知系统(替换TIT22飞信)** |
| ═══ | ═══ | **═══ PB→Python 等价重构完成线 ═══** |
| 阶段5 | 重构后近期 | **[Tier-2]** 租金/费用自动结算 + 财务应收应付 + 客户自助服务门户 |
| 阶段6+ | 重构后中远期 | **[Tier-3]** 移动端适配 / MES生产制造 / IoT数据监控（按业务需求启动） |

> **重要**：Tier-1 功能（SLA/合同/发票/通知）已确认在重构阶段同步开发，不可遗漏。
