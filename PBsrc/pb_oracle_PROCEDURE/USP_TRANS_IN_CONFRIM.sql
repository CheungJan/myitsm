procedure usp_trans_in_confrim(as_type   char,
                                                 as_id     number,
                                                 as_userid char,
  ...
  ******************************************************/
  If v_type = '1' Then
    ...
    IF substr(v_tfid,1,2) = 'PL' THEN
       ...
       --FOR V_PLAN IN (SELECT * FROM PLAN_CUST WHERE PLANNO = v_tfid) LOOP
 ...
 --整机逆流--作废
  ...
  if v_type = '3' THEN
    ...
    select changetype, serialno, old_serialno, operflg, INSERT_TIME, case_no
     ...
     ----beg  新加按计划取机
      select store_id,REQUSET_PAPER_ID into v_custcd,v_tfid
	        from tit10_maintenanceday
	       where maintenance_id = v_caseno;

     IF v_tftype = '5' AND v_tfid IS NOT NULL THEN
        usp_plan_confrim(as_type,as_id,as_userid,as_return);
        IF as_return <> 'OK' THEN
           return;
        END IF;
     ...
     IF v_tftype = '5' AND v_tfid is null THEN
