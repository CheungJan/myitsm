...
class TestRecycleTask11d:
    ...
    def test_recycle_close_writes_r_track_and_invalidates_rl(self, app: Flask) -> None:
        ...
        with app.app_context():
            ...
            eids = (
                ...
                .filter(Eid.eid.in_(["EIDRCY0000001", "EIDRCY0000002"]))
                .all()
            )
            assert all(e.sflg == "3" for e in eids), "回收设备 sflg 应为 '3'（待检）"
    ...
    def test_recycle_close_auto_creates_inbound(self, app: Flask) -> None:
        """回收任务关单：配置了 recycle_return_whcd 且明细为自有资产时，
        自动创建回收入库草稿（IV=7）。
        """
        with app.app_context():
            _seed_customer("CUST001", "CARD001")
            _seed_eid("EIDRC00000050")
            eid_rec = db.session.query(Eid).filter(Eid.eid == "EIDRC00000050").first()
            eid_rec.asset_owner = "02"  # 自有资产
            _seed_rl("CUST001", "EIDRC00000050")
            task = _seed_recycle_task(asset_ids=["EIDRC00000050"])
            db.session.commit()
            ...
            stock_in = (
                ...
                .filter(
                    StockIn.refbillid == task.recycle_id,
                    StockIn.invtyp == "7",
                )
                .first()
            )
    ...
    def test_recycle_close_no_whcd_no_inbound(self, app: Flask) -> None:
        """回收任务关单：未配置 recycle_return_whcd 时不创建入库草稿。"""
        with app.app_context():
            _seed_customer("CUST001", "CARD001")
            _seed_eid("EIDRC00000060")
            eid_rec = db.session.query(Eid).filter(Eid.eid == "EIDRC00000060").first()
            eid_rec.asset_owner = "02"
            _seed_rl("CUST001", "EIDRC00000060")
            task = _seed_recycle_task(asset_ids=["EIDRC00000060"])
            db.session.commit()
            ...
            stock_in = (
                db.session.query(StockIn)
                .filter(StockIn.refbillid == task.recycle_id)
                .first()
            )
